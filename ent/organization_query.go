...
type OrganizationQuery struct {
	...
	withOwnedPortfolios *PortfolioQuery
	withPeople          *PersonQuery
	withSubsidiaries    *OrganizationQuery
	withParentOrg       *OrganizationQuery
	withFKs             bool
...
// QueryOwnedPortfolios chains the current query on the "owned_portfolios" edge.
func (_q *OrganizationQuery) QueryOwnedPortfolios() *PortfolioQuery {
...
// QuerySubsidiaries chains the current query on the "subsidiaries" edge.
func (_q *OrganizationQuery) QuerySubsidiaries() *OrganizationQuery {
...
// QueryParentOrg chains the current query on the "parent_org" edge.
func (_q *OrganizationQuery) QueryParentOrg() *OrganizationQuery {
	query := (&OrganizationClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
func (_q *OrganizationQuery) Clone() *OrganizationQuery {
	...
	return &OrganizationQuery{
		...
		withOwnedPortfolios: _q.withOwnedPortfolios.Clone(),
		withPeople:          _q.withPeople.Clone(),
		withSubsidiaries:    _q.withSubsidiaries.Clone(),
		withParentOrg:       _q.withParentOrg.Clone(),
...
// WithOwnedPortfolios tells the query-builder to eager-load the nodes that are connected to
// the "owned_portfolios" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *OrganizationQuery) WithOwnedPortfolios(opts ...func(*PortfolioQuery)) *OrganizationQuery {
	query := (&PortfolioClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withOwnedPortfolios = query
	return _q
}
...
// WithSubsidiaries tells the query-builder to eager-load the nodes that are connected to
// the "subsidiaries" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *OrganizationQuery) WithSubsidiaries(opts ...func(*OrganizationQuery)) *OrganizationQuery {
	query := (&OrganizationClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withSubsidiaries = query
	return _q
}

// WithParentOrg tells the query-builder to eager-load the nodes that are connected to
// the "parent_org" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *OrganizationQuery) WithParentOrg(opts ...func(*OrganizationQuery)) *OrganizationQuery {
	query := (&OrganizationClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withParentOrg = query
	return _q
}
...
func (_q *OrganizationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Organization, error) {
	var (
		...
		_spec       = _q.querySpec()
		loadedTypes = [4]bool{
			_q.withOwnedPortfolios != nil,
			_q.withPeople != nil,
			_q.withSubsidiaries != nil,
			_q.withParentOrg != nil,
		}
	)
	if _q.withParentOrg != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := _q.withOwnedPortfolios; query != nil {
		if err := _q.loadOwnedPortfolios(ctx, query, nodes,
			func(n *Organization) { n.Edges.OwnedPortfolios = []*Portfolio{} },
			func(n *Organization, e *Portfolio) { n.Edges.OwnedPortfolios = append(n.Edges.OwnedPortfolios, e) }); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withSubsidiaries; query != nil {
		if err := _q.loadSubsidiaries(ctx, query, nodes,
			func(n *Organization) { n.Edges.Subsidiaries = []*Organization{} },
			func(n *Organization, e *Organization) { n.Edges.Subsidiaries = append(n.Edges.Subsidiaries, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withParentOrg; query != nil {
		if err := _q.loadParentOrg(ctx, query, nodes, nil,
			func(n *Organization, e *Organization) { n.Edges.ParentOrg = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *OrganizationQuery) loadOwnedPortfolios(ctx context.Context, query *PortfolioQuery, nodes []*Organization, init func(*Organization), assign func(*Organization, *Portfolio)) error {
	...
	query.Where(predicate.Portfolio(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(organization.OwnedPortfoliosColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.organization_owned_portfolios
		if fk == nil {
			return fmt.Errorf(`foreign-key "organization_owned_portfolios" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "organization_owned_portfolios" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (_q *OrganizationQuery) loadSubsidiaries(ctx context.Context, query *OrganizationQuery, nodes []*Organization, init func(*Organization), assign func(*Organization, *Organization)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Organization)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Organization(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(organization.SubsidiariesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.organization_subsidiaries
		if fk == nil {
			return fmt.Errorf(`foreign-key "organization_subsidiaries" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "organization_subsidiaries" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *OrganizationQuery) loadParentOrg(ctx context.Context, query *OrganizationQuery, nodes []*Organization, init func(*Organization), assign func(*Organization, *Organization)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Organization)
	for i := range nodes {
		if nodes[i].organization_subsidiaries == nil {
			continue
		}
		fk := *nodes[i].organization_subsidiaries
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "organization_subsidiaries" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
