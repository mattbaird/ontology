...
func (_q *ApplicationQuery) QueryResultingLease() *LeaseQuery {
...
func (_q *ApplicationQuery) QueryApplicant() *PersonQuery {
...
// the "resulting_lease" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *ApplicationQuery) WithResultingLease(opts ...func(*LeaseQuery)) *ApplicationQuery {
	query := (&LeaseClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withResultingLease = query
	return _q
}
...
func (_q *ApplicationQuery) WithApplicant(opts ...func(*PersonQuery)) *ApplicationQuery {
	query := (&PersonClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *ApplicationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Application, error) {
	...
	if query := _q.withApplicant; query != nil {
		if err := _q.loadApplicant(ctx, query, nodes, nil,
			func(n *Application, e *Person) { n.Edges.Applicant = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *ApplicationQuery) loadResultingLease(ctx context.Context, query *LeaseQuery, nodes []*Application, init func(*Application), assign func(*Application, *Lease)) error {
	...
	for i := range nodes {
		if nodes[i].lease_application == nil {
			continue
		}
		fk := *nodes[i].lease_application
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "lease_application" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *ApplicationQuery) loadApplicant(ctx context.Context, query *PersonQuery, nodes []*Application, init func(*Application), assign func(*Application, *Person)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Application)
	for i := range nodes {
