...
func (_q *AccountQuery) QueryEntries() *LedgerEntryQuery {
...
func (_q *AccountQuery) QueryBankAccounts() *BankAccountQuery {
...
func (_q *AccountQuery) WithEntries(opts ...func(*LedgerEntryQuery)) *AccountQuery {
	query := (&LedgerEntryClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *AccountQuery) WithBankAccounts(opts ...func(*BankAccountQuery)) *AccountQuery {
	query := (&BankAccountClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *AccountQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Account, error) {
	...
	if query := _q.withEntries; query != nil {
		if err := _q.loadEntries(ctx, query, nodes,
			func(n *Account) { n.Edges.Entries = []*LedgerEntry{} },
			func(n *Account, e *LedgerEntry) { n.Edges.Entries = append(n.Edges.Entries, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withBankAccounts; query != nil {
		if err := _q.loadBankAccounts(ctx, query, nodes,
			func(n *Account) { n.Edges.BankAccounts = []*BankAccount{} },
			func(n *Account, e *BankAccount) { n.Edges.BankAccounts = append(n.Edges.BankAccounts, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *AccountQuery) loadChildren(ctx context.Context, query *AccountQuery, nodes []*Account, init func(*Account), assign func(*Account, *Account)) error {
	...
	for _, n := range neighbors {
		fk := n.account_children
		if fk == nil {
			return fmt.Errorf(`foreign-key "account_children" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "account_children" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (_q *AccountQuery) loadParent(ctx context.Context, query *AccountQuery, nodes []*Account, init func(*Account), assign func(*Account, *Account)) error {
	...
	nodeids := make(map[uuid.UUID][]*Account)
	for i := range nodes {
		if nodes[i].account_children == nil {
			continue
		}
		fk := *nodes[i].account_children
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "account_children" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *AccountQuery) loadEntries(ctx context.Context, query *LedgerEntryQuery, nodes []*Account, init func(*Account), assign func(*Account, *LedgerEntry)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Account)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
...
func (_q *AccountQuery) loadBankAccounts(ctx context.Context, query *BankAccountQuery, nodes []*Account, init func(*Account), assign func(*Account, *BankAccount)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Account)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.BankAccount(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(account.BankAccountsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.account_bank_accounts
		if fk == nil {
			return fmt.Errorf(`foreign-key "account_bank_accounts" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "account_bank_accounts" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
